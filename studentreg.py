...
def register_admin():
    ...
    if conn:
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO etudiants (name, email, password) VALUES (%s, %s, %s)",
                           (name, email, password))
            conn.commit()
...
def open_student_registration():
    ...
    # Title Label
    tk.Label(registration_window, text="Student Registration", font=("Times new roman", 16), fg="blue").pack(pady=10)
...
def main():
    ...
    btn_admin_registration = tk.Button(root, text="Student Registration", command=open_admin_registration,
                                       width=30)

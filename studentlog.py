...
from menu import create_main_window
...
def open_student_login():
    ...
    tk.Label(login_window, text="Student Login", font=("Times new roman", 14)).pack(pady=10)
    ...
    tk.Button(login_window, text="Login", command=lambda: login_student(login_window), width=15).pack(pady=20)


# Function to handle the login of an administrator
def login_student(login_window):
    ...
    if conn:
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT * FROM etudiants WHERE email = %s AND password = %s", (email, password))
...
def open_main_app(login_window):
    ...
    # Title Label
    tk.Label(login_window, text="Student Login", font=("Times new roman", 14), fg="blue").pack(pady=10)
    ...
    # Login Button
    tk.Button(login_window, text="Login", command=login_student, width=15).pack(pady=20)
...
def main():
    ...
    # Title Label
    title_label = tk.Label(root, text="Welcome to the University Management System", font=("Times new roman", 14))
    ...
    btn_admin_login = tk.Button(root, text="Student Login", command=open_student_login, width=30)
